...
static const regex tile_rxcx_re(R"(x(\d+)y(\d+))");
...
pair<int, int> get_row_col_pair_from_chipsize(string name, pair<int, int> chip_size, int bias) {
    ...
    if(match) {
        return make_pair(stoi(m.str(2)), stoi(m.str(1)));
    } else {
        throw runtime_error(fmt("Could not extract position from " << name));
    }
}

Tile::Tile(Tang::TileInfo info, Tang::Chip &parent) : info(info), cram(parent.cram.make_view(info.frame_offset,
                                                                                                   info.bit_offset,
                                                                                                   info.num_frames,
                                                                                                   info.bits_per_frame)) {}
