// Ported from libtrellis and adapted
...
DeviceLocator find_device_by_idcode(uint32_t idcode) {
    auto found = find_device_generic([idcode](const string &n, const pt::ptree &p) -> bool {
        UNUSED(n);
        boost::optional<string> idcode_db = p.get_optional<string>("deviceid");
        if (idcode_db) {
            return parse_uint32(*idcode_db) == idcode;
        } else {
            return false;
        }
    });
    if (!found)
        throw runtime_error("no device in database with deviceid " + uint32_to_hexstr(idcode));
    return *found;
}
...
ChipInfo get_chip_info(const DeviceLocator &part) {
    ...
    ci.pad_bits_after_frame = dev.get<int>("pad_bits_after_frame", 0);
    ci.pad_bits_before_frame = dev.get<int>("pad_bits_before_frame", 0);
    ci.idcode = parse_uint32(dev.get<string>("deviceid", "0"));
    ci.max_row = dev.get<int>("max_row");
    ci.max_col = dev.get<int>("max_col");
    ci.col_bias = dev.get<int>("col_bias", 0);
    return ci;
}
...
vector<TileInfo> get_device_tilegrid(const DeviceLocator &part) {
    ...
    string tilegrid_path = db_root + "/" + part.family + "/" + part.device + "/tilegrid.json";
    {
        ...
        for (const pt::ptree::value_type &tile : tg) {
            ...
            ti.name = tile.first;
            ti.num_frames = size_t(tile.second.get<int>("cols", 1));
            ti.bits_per_frame = size_t(tile.second.get<int>("rows", 1));
            ti.bit_offset = size_t(tile.second.get<int>("start_bit", 0));
            ti.frame_offset = size_t(tile.second.get<int>("start_frame", 0));
