...
#ifndef LIBTANG_BITDATABASE_HPP
#define LIBTANG_BITDATABASE_HPP
...
The BitDatabase keeps track of what each bit in a tile does. Unlike other databases, this database is mutable from
 within libtang, for use during fuzzing.
 */
...
template<>
struct hash<Tang::ConfigBit>
{
public:
    inline size_t operator()(const Tang::ConfigBit &bit) const
    {
        hash<int> hash_i_fn;
        hash<bool> hash_b_fn;
...
public:
    ...
    void remove_mux_arc(const string &source, const string &sink);
    ...
    // This should not be used, but is required for PyTang
